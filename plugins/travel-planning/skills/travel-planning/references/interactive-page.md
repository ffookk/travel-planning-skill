...
最终页面采用“日期与时间节点 + 右侧卡片”的单列时间轴主结构，并满足以下规则。
...
- 餐饮：顶部显示时间窗、主选理由和切换规则；主选使用完整卡片并始终展开。备选卡默认折叠，折叠标题仍显示备选编号、店名、菜系和清晰的展开提示，支持键盘与触摸操作；展开后显示完整证据、路线和操作按钮。PC 端只有 1 个备选时占满整行，有 2 个备选时才并排；窄屏始终单列。每个候选显示有来源的图片或相册链接、名称/分店、菜系、特色菜、人均、分平台评分与评价量、近期社区共识、营业、排队/预约、停车、从上一站/去下一站的距离时间、额外绕行和平台链接。不得把小红书互动数或多个平台评分合成无来源的总分。
