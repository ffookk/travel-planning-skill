...
def render_restaurant_candidate(
    ...
    role_class = " primary" if role == "主选" else ""
    return f'''<article class="restaurant-candidate{role_class}"><div class="restaurant-candidate-head"><span>{esc(role)}</span><div><h4>{esc(restaurant.get("name"))}</h4><p>{esc(" · ".join(str(x) for x in restaurant.get("cuisine") or []))}</p></div></div>{visual}
      ...
      {render_actions(merge_actions(preview_actions, restaurant.get("action_links") or [], snapshot.get("action_links") or [], route_actions))}</article>'''
...
def render_event(
    ...
    return f'''<article class="event event-{esc(kind)}" id="event-{idx}">
      <div class="time"><strong>{esc(event.get("time"))}</strong>{end_time}</div>
...
def render_day(
    ...
    return f'''<section class="day" id="day-{day_idx}">
      <div class="day-heading"><div><span class="eyebrow">{esc(day.get("label") or day.get("date"))}</span><h2>{esc(day.get("title"))}</h2>{weather}{cost}</div><p>{esc(day.get("summary"))}</p></div>
...
def build(data: dict[str, Any]) -> str:
    validate_data(data)
    trip, days = data.get("trip") or {}, data.get("days") or []
    tabs = "".join(f'<button class="day-tab{(" active" if i == 0 else "")}" data-target="day-{i}" aria-pressed="{str(i == 0).lower()}"><strong>{esc(d.get("label") or f"D{i+1}")}</strong><span>{esc(d.get("date"))}</span></button>' for i, d in enumerate(days))
    planning = data.get("planning") or {}
    ...
    assumptions = render_list(trip.get("assumptions") or [], "assumption-list")
...
@media(max-width:620px){{.hero-inner{{padding:14px 14px 0}}h1{{font-size:27px}}.meta{{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}main{{padding:0 10px 60px;margin-top:18px}}.overview,.planning{{margin:0 4px 20px}}.research-grid,.summary-grid{{grid-template-columns:1fr}}.day-heading{{margin-left:70px;padding-right:5px;display:block}}.day-heading p{{text-align:left;margin-top:4px}}.event{{grid-template-columns:57px 18px 1fr}}.time{{padding-right:6px}}.time strong{{font-size:12px}}.dot{{width:21px;height:21px;font-size:9px}}.card{{margin-left:8px;padding:15px 13px;border-radius:15px}}.card h3{{font-size:17px}}.image-strip,.image-strip img{{height:118px;min-height:118px}}}}
...
</style></head><body><header class="hero"><div class="hero-inner"><span class="kicker">旅行计划</span><h1>{esc(trip.get("title"))}</h1><p class="trip-subtitle">{esc(trip.get("subtitle"))}</p><p class="meta">{esc(meta)}</p><nav class="tabs" aria-label="按日期跳转">{tabs}</nav></div></header>
<main>{f'<section class="overview"><h2>规划说明</h2>{assumptions}</section>' if assumptions else ''}{day_html}{f'<details class="sources"><summary>信息来源（{len(data.get("sources") or [])} 条）</summary><ul>{sources_html}</ul></details>' if sources_html else ''}<p class="footer">最后更新：{esc(trip.get("updated_at") or "未注明")} · 出发前请再次核对时刻、价格与开放状态</p></main>
<script>
const tabs=[...document.querySelectorAll('.day-tab')];
tabs.forEach(b=>b.addEventListener('click',()=>document.getElementById(b.dataset.target)?.scrollIntoView({{behavior:'smooth'}})));
const ss=[...document.querySelectorAll('.day')];
if('IntersectionObserver'in window){{
  const dayObserver=new IntersectionObserver(es=>{{
    const e=es.filter(x=>x.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];
    if(!e)return;
    tabs.forEach(b=>{{const on=b.dataset.target===e.target.id;b.classList.toggle('active',on);b.setAttribute('aria-pressed',String(on))}})
  }},{{rootMargin:'-35% 0px -55%',threshold:[0,.25,.6]}});
  ss.forEach(s=>dayObserver.observe(s));
}}
...
const unloadMap=f=>{{if(f.src&&f.src!=='about:blank')f.src='about:blank'}};
if('IntersectionObserver'in window){{
  const mapObserver=new IntersectionObserver(es=>es.forEach(e=>e.isIntersecting?loadMap(e.target):unloadMap(e.target)),{{rootMargin:'500px 0px'}});
