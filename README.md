...
GitHub Actions 会在 `main` 推送和每个 PR 上运行 Python 3.10 / 3.14 全量单元测试、Node.js 24 前端构建及已提交资源一致性检查，以及 marketplace ZIP 完整性和清单检查。也可手动触发 `Checks` 工作流。

单元测试使用空的临时数据源配置并阻止真实网络连接；不需要本地凭证文件，也不会执行供应商查询、登录或预订。真实数据源的连通性仍需在本机显式运行 `preflight` 验证。
